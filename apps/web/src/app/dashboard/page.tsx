...
import { Plus, LogIn, LogOut, Code2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const res = await api.post('/sessions', createForm);
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    setJoining(true);
    try {
      const res = await api.post('/sessions/join', { roomCode: joinCode.toUpperCase() });
      router.push(`/session/${res.data.id}`);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        {/* Session list */}
        <div>
          <h2 className="font-semibold text-gray-900 mb-4">Your sessions</h2>
          {sessions.length === 0 ? (
            <p className="text-gray-500 text-sm">No sessions yet. Create or join one above.</p>
          ) : (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      Code: <span className="font-mono font-bold">{s.roomCode}</span> · {s.language}
                    </p>
                  </div>
